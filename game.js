...
function showSuccess(){const a=animals[stage],nextButton=$("nextBtn"),nextLabel=stage===animals.length-1?"לחגיגה הגדולה <span>←</span>":"לחיה הבאה <span>←</span>";$("successAnimal").textContent=a.emoji;$("successTitle").textContent=`ה${a.name} קיבל ${a.foodName}!`;$("shtuzText").textContent=currentShtuz;nextButton.innerHTML="מקשיבים לשטוז…";nextButton.disabled=true;show("success");playRecorded("voice-05.mp3",narration.wellDone,()=>playRecorded(`shtuz-${String(currentShtuzNumber).padStart(2,"0")}.mp3`,currentShtuz,()=>{nextButton.innerHTML=nextLabel;nextButton.disabled=false;}));}
